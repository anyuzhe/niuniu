...
### 2026-09-25｜[CODE] 日内做T 第四轮：外部资料与盘口数据（研究，无产品代码改动）

- 按用户要求检索 A 股日内交易的学术与券商资料，并用 `gst_intraday` 的五档盘口（2019-05..2020-06）做实验，记录见[日内做T 策略研究 §9](../archive/intraday/20260925-日内做T策略研究.md)，脚本在 `scripts/research/gst_intraday_t0/`（`gao*.py`、`orderbook/`）。
- 大盘日内动量在 16 只平均上复现（2023–2024 样本外 R² 10%），但幅度太小，不能直接做个股 T。盘口失衡短周期预测力强（30 秒相关约 0.2）但小于一个价位；保守排队模型下挂单做 T 亏损；真实盘口显示回测的“下一分钟开盘价减 1 个价位”比直接按买一卖出差约 4.7 bp，即回测成本偏保守，挂单进场与盘口择时基本无改善。
- 测试：未改产品代码；脚本为一次性研究，只在虚拟机上运行。

### 2026-09-25｜[CODE] 日内做T 第五轮：1 分钟缠论买卖点 + 盘口确认（研究，无产品代码改动）

- 按用户提出的思路，用仓库固定的缠论核心逐根 1 分钟 K 线增量计算买卖点（只用确认时刻，无未来函数），16 只股票 2019–2024 年共 9,799 个点，按底仓做T 进出、收盘竞价/反向点/止损三种出场评估；再在 2019-05..2020-06 用盘口失衡与主动成交做确认。记录见[日内做T 策略研究 §10](../archive/intraday/20260925-日内做T策略研究.md)，脚本在 `scripts/research/gst_intraday_t0/chan/`。
- 结果：各类买卖点每笔扣费后约 −5 至 −37 bp，止损更差；大盘同向过滤在检验期不成立；盘口确认无改善，主动成交确认更差。未改产品代码。

### 2026-09-25｜[CODE] 日内做T 第六轮：用盘口提前判断一买（研究，无产品代码改动）

- 按用户提出的思路测试“用盘口提前判断 1 分钟一买”。事后完美预判一买位置的收益上限：拿到收盘每笔 +1.7 bp（2019–2022）/ −7.7 bp（2023–2024），其他出场方式也在 −15 至 +8 bp；实时下跌段创新低买入约 −34 bp，加盘口衰竭条件 −27 bp，信号只有约 1% 落在真一买附近。记录见[日内做T 策略研究 §11](../archive/intraday/20260925-日内做T策略研究.md)，脚本 `scripts/research/gst_intraday_t0/chan/b1_*.py`。未改产品代码。

### 2026-09-25｜[CODE] 日内做T 第七轮：全市场 5 分钟数据与大盘择时（研究，无产品代码改动）

- 用 READY 的 `bars_min5_baostock_raw` 临时算全市场每 5 分钟等权涨跌与上涨家数占比（研究口径），替换“16 只平均”并把上午打分扩展到每年成交额前 500 只，逐年滚动验证 2021–2024。记录见[日内做T 策略研究 §12](../archive/intraday/20260925-日内做T策略研究.md)，脚本 `scripts/research/gst_intraday_t0/market5m/`。
- 结果：全市场版上午打分四年都为正（+26 至 +46 bp/笔）；个股选择信号很弱（相关 0.01–0.02），大盘择时稳定（相关 0.15–0.19）；10:00 预计走弱的日子先卖后买，每年约 8–26 天、每笔 +39 至 +80 bp、当天盈利约 66%，t≈1.6–2.1。
- 已写给数据侧的需求：全市场日内情绪序列（历史 + 盘中）、主要指数分钟线、更多股票的日内数据（Project 文档 `claude/niuniu-data-request-intraday-market.md`）。未改产品代码。

### 2026-09-26｜[CODE] 日内做T 第八轮：数据侧新数据上的样本外检验（研究，无产品代码改动）

- 数据侧开放 `market_intraday_breadth`、`tdx_kline_min1`（2026-05-21..09-24）与指数分钟线；清单解析正常（READY 46 项）。
- 把 2020–2024 定下的大盘择时与个股上午打分原样放到 2026-05..09（每年成交额前 500 只）：大盘预测与实际相关 −0.21，择时各门槛每笔 +3.8 至 −24 bp；个股打分每笔 −15 至 −88 bp。指数 5 分钟（2024-09 起）的简单日内动量 2025 年多为正（0.09–0.25），2026 年下半年降到约 0，等权口径 2026-05..09 为 −0.11。记录见[日内做T 策略研究 §13](../archive/intraday/20260925-日内做T策略研究.md)，脚本 `scripts/research/gst_intraday_t0/market5m/oos2026.py`、`index_mom.py`。
- 结论：大盘择时暂不做成产品策略；等 5 分钟情绪（2020 年起）与盘中实时开放后按滚动窗口重估。

### 2026-09-26｜[CODE] 日内做T 第九轮：正式 5 分钟全市场情绪上的大盘择时重估（研究，无产品代码改动）

- 数据侧开放 `market_intraday_breadth_5m`（2020-01..2026-09，含退市股，READY）；`market_intraday_breadth_live` 仍 NOT_READY。清单解析正常（READY 47 项）。
- 逐年滚动的 10:00 大盘择时：预测相关 2021–2025 为 +0.08 至 +0.19，2026 为 −0.12；门槛 15 bp 共 110 天每笔 +27.8 bp（t=1.65），2025 年约 −1 bp，2026 年 +13 bp。近 120 日相关开关反而变差（+19.0 bp，2026 年 −24 bp）。记录见[日内做T 策略研究 §14](../archive/intraday/20260925-日内做T策略研究.md)，脚本 `scripts/research/gst_intraday_t0/market5m/timing2.py`。
- 结论：大盘择时不做成自动交易策略；未改产品代码。

### 2026-09-26｜[CODE] 日内做T 第十轮：平台突破、放量突破、大盘随动（研究，无产品代码改动）

- 按用户要求，在每年成交额前 500 只股票的 5 分钟线（2020-01..2026-09）上测试日内平台突破（324 组）、多日平台突破（324 组）、放量突破（54 组）和大盘随动（36 组），大盘用 `market_intraday_breadth_5m`；2020–2022 选参数，2023–2024 检验，2025–2026 为新样本。
- 结果：前三类在训练期没有一组扣费后为正，最好的每笔约 −8 bp（胜率约 39%，赔率 1.2–1.4），毛收益只有 0 至 +7 bp，不够覆盖约 20 bp 的来回成本；大盘随动训练期最好 +0.5 bp、检验期 −22 bp。记录见[日内做T 策略研究 §15](../archive/intraday/20260925-日内做T策略研究.md)，脚本 `scripts/research/gst_intraday_t0/breakout5m/`。未改产品代码。

### 2026-09-26｜[CODE] 日内做T 第十一轮：突破后波段出场（研究，无产品代码改动）

- 按用户意见把突破后的卖出改为“一波就走”：止盈、止损、从高点回撤、限时等 162 种卖法，5 分钟线 2020–2026 共 1,620 组，训练期没有一组为正；1 分钟线（2026-05..09）384 种卖法 × 4 种买点也没有一种为正。突破后 30 分钟内最大涨幅和最大跌幅同时比全体股票大（如放量破昨高 +87 / −76 bp，全体 +59 / −50），买点没有方向优势。记录见[日内做T 策略研究 §16](../archive/intraday/20260925-日内做T策略研究.md)，脚本 `breakout5m/exits.py`、`aggx.py`、`m1.py`。未改产品代码。

### 2026-09-26｜[CODE] 日内做T 第十二轮：开盘先卖、盘中反转、大幅高开、板块补涨（研究，无产品代码改动）

- 在每年 500 只的 5 分钟线（2020–2026）上测试反向做法：每天开盘先卖（按开盘前可知的特征分组）、10:00/10:30 横截面反转、大幅高开先卖（计入收盘封板买不回，改为次日开盘买回）、申万一级板块补涨。
- 结果：开盘先卖各分组、盘中反转、板块补涨都为负；高开 ≥7% 先卖、涨 5% 止损买回合并每笔 +94 bp，但 t≈1.3、单笔尾部亏损大、封板买不回占 26–34%，不作为策略。记录见[日内做T 策略研究 §17](../archive/intraday/20260925-日内做T策略研究.md)，脚本在 `scripts/research/gst_intraday_t0/breakout5m/`。未改产品代码。

### 2026-09-26｜[CODE] 日内做T 第十三轮：3 秒级冲刺剥头皮（研究，无产品代码改动）

- 按用户思路，用 `gst_intraday` 约 3 秒一张的五档盘口（2019-05..2020-06，16 只）测试“几笔放量/平台突破就按卖一买入，冲刺停了按买一卖出”，136 组规则全部为负（最好每笔 −12 bp）。冲刺确实延续（1 分钟内中间价多涨 3–14 bp，随机约 0），但冲刺时买入价差 1–2 个价位，加约 10 bp 费用后每笔 −8 至 −40 bp；低价股一个价位够付费用，但冲刺后只多走约 0.2–0.5 个价位。记录见[日内做T 策略研究 §18](../archive/intraday/20260925-日内做T策略研究.md)，脚本 `scripts/research/gst_intraday_t0/tick/`。未改产品代码。

### 2026-09-26｜[CODE] 日内做T 第十四轮：隔夜T 与 ETF 做T（研究，无产品代码改动）

- 隔夜T：每年 500 只 2020–2026，收盘到次日开盘不扣费平均每天约 −8 至 −11 bp，每年为负，扣费后每笔 −14 至 −36 bp，各分组均为负。
- ETF 做T（指数 5 分钟线代替，成本 10 bp）：“10:00 指数跌 ≥0.5% 先卖、收盘买回”在沪深300、中证500、中证1000、创业板指、上证指数的 2024-09..2025-09 与 2025-10..2026-09 两段基本都为正（每笔约 +8 至 +45 bp），但每段几十次、不显著；早盘涨了先买第二年转负；开盘区间突破为负。记录见[日内做T 策略研究 §19](../archive/intraday/20260925-日内做T策略研究.md)，脚本 `breakout5m/overnight.py`、`onagg.py`、`etf.py`。未改产品代码。
