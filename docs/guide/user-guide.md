...
打开后左侧是 11 个日常工作台，每个回答一个问题；研究、治理和开发工具收在左下角“专业模式”开关里（设置按工作空间保存在 `artifacts/_desktop/ui.json`）。

| 页面 | 回答的问题 | 当前状态 |
|---|---|---|
| 今日市场 | 市场整体怎么样：涨跌家数、涨停跌停、连板高度、成交额、昨日涨停溢价、两融，并给出近一年分位 | 已上线（盘后） |
| 主线方向 | 什么方向最强：领涨行业、连续进前五天数、涨停原因集中度 | 已上线（盘后） |
| 盘中板块 | 交易时间里同花顺概念/行业板块的涨幅、跌幅、成交额排行；点开板块看成分股的现价、涨跌幅、涨停/炸板/跌停；可问 AI 解读 | 已上线（数据侧 2026-09-28 盘中实测延迟和限流） |
| 日内做T | 手里有底仓时，当天高抛低吸能不能赚钱：用 16 只股票 2019–2024 年的 1 分钟数据回测研究所得的日内打分等策略和文章里的三种日内策略（扣佣金、印花税、滑点，分训练期/检验期），并逐日回放分时走势和每笔模拟交易 | 已上线（数据侧 `gst_intraday`，只有 16 只股票，研究用） |
...
| 复盘验证 | 在个股报告里保存的判断（看多/观望/看空、5/10/20个交易日、可设失效价和目标价），按之后的真实走势自动核对，统计准确率、顺向收益和相对全市场的超额，并分“我的判断/AI 的判断/大V观点”对比 | 已上线 |
| 大V复盘 | 粘贴看过的大V复盘文章，用 AI（或手动）提炼出能核对的大盘/个股/方向观点，确认后自动核对，按作者统计准确率；可一键让 AI 汇总最新观点的共识与分歧 | 已上线（文章需手动粘贴） |
| AI 助手 | 自然语言提问；默认“日常”模式只带少量工具（市场、个股、我的股票、复盘判断、公告/研报/新闻/财报；数据清单开放扶摇且配置了密钥时再加5个扶摇查询：证券消歧、个股快照、板块、短线热度/龙虎榜/涨跌停、估值财务），问到具体股票时自动附上实时报价（扶摇为主、公开行情交叉校验），专业模式下默认“研究”模式（全部研究工具）；各页“问 AI”会把当前页面数据填进输入框，确认后再发送 | 已上线 |
| 数据中心 | 牛牛能用哪些数据：数据清单里全部文件、数据库、查询接口的内容、覆盖日期、状态和用法，可搜索筛选；更新状态（各数据集最新日期、健康、封存与核对）、文件预览与接口试查、在页面上发起数据侧的更新与封存任务（先看计划再执行） | 已上线 |

专业模式下的页面：交易台（原“今日交易”）、主题矩阵（原“主线市场”）、股票决策档案（原“股票中心”）、持仓计划、决策复盘（原“复盘中心”）、AI 团队、研究实验室、开发工作台、系统中心；开发工作台新增六职责需求入口，其余功能沿用原入口。菜单接线见 [app.py](../../src/quantlab/desktop/app.py) 的 `PAGES`。

### 开发工作台：六职责协作

启用左下角“专业模式”，进入“开发工作台”，点击“六角色模型”设置 LEAD（总控）、DATA（数据）、CORE（研究交易）、AI（助手运行时）、APP（产品界面）、QA（独立测试审核）。角色可以使用相同模型；支持 Codex CLI、Responses、Chat Completions 和本地 Pi SDK（`pi_sdk`）协议。默认继承当前模型设置，之后可逐角色修改模型 ID、推理强度、地址、超时、上下文与工具预算；密钥只填环境变量名，不填写真实密钥。保存配置不会调用模型。

使用本机 Pi：协议选 `pi_sdk`，模型填写 `provider/model`（例如 `openai-codex/gpt-6-luna`）；Pi 路径可留空自动发现，也可指定本机 `pi` 可执行文件。先在 Pi 中完成登录。适配通过已安装的 Pi `ModelRuntime` 调用模型及其自定义 provider，不加载 Pi AgentSession、内置工具、插件、skills 或项目指令；工具始终由牛牛按角色授权。它是本地 Pi 客户端调用模型服务，不表示模型在本机离线推理。配置一个角色后可点“复制当前配置到全部角色”，再保存；修改配置不替换旧任务的冻结模型。

点击“＋ 开发需求”，用自然语言描述目标、约束和不希望改动的内容，再点击“生成计划（调用总控模型）”。这一步会向所选总控模型发送需求和必要代码，但只读分析，不创建开发任务、不改代码。模型必须通过正式工具交付计划，页面显示精确文件负责人、验收条件、冻结测试、接口依赖、六角色模型和敏感文件提示。

审阅后点击“确认计划并开始开发”，系统核对需求与 Git SHA，创建隔离工作区并自动进入总控调度。只派发需求实际需要的领域；默认最多两名实施者同时写不同文件，随后 QA 测试、QA 独立审核、总控验收。角色表显示实际子任务、阶段、模型、状态和执行次数；下方执行记录保留失败与返工。`READY_FOR_HUMAN` 仅表示等待人工合并，不表示主工作区已改变。

修改输入框会使旧计划失效；计划 30 分钟后过期，生成后主分支变化也需重新规划。“请求停止”阻止后续调用和派发，正在执行的测试须退出或超时，隔离改动不丢弃。异常中断后重新进入可查看持久任务，点击“运行 / 继续”交总控检查并接续；不承诺关闭客户端后继续开发。

完成后先“查看 Diff”，再“确认合并”。合并前重新核对最终指纹、测试、审核、主分支基线和干净状态；验收后代码有变化或主分支前进时拒绝合并，不能强行沿用旧验收。产品只创建本地 Git 提交，**不自动 push、部署、采集数据或运行交易**。“高级 DevTask”保留原手工指定路径/测试入口，旧任务不自动升级为六职责。

开发测试使用宿主 Python、离屏 Qt 和临时数据根，**不是操作系统沙箱**；只能批准已审阅的离线隔离测试，不要用它执行不可信仓库或接触真实数据/凭据的测试。各角色权限、文件归属与失败处理见 [开发归属说明](../development/ownership.md)。当前验收使用脚本化模型传输、临时仓库和离屏界面，不等同于真实大模型完成复杂业务开发。

命令行同源入口（工作空间须已存在，模型调用需显式确认）：

```bash
# 只读规划；标准输出是 {ok, data}，data 才是可审批的计划对象。
.venv/bin/python -m quantlab.agent.dev_studio_cli --repo-root . --output ./artifacts \\
  --plan-request --request '描述你的开发需求' --allow-model
# 将上一步返回的 data 原样保存到 plan.json，审阅后创建任务；此命令本身不开始执行。
.venv/bin/python -m quantlab.agent.dev_studio_cli --repo-root . --output ./artifacts \\
  --approve-plan --spec-json plan.json --confirm
# 用返回的真实 task_id 运行；只在隔离工作区写代码，结束后仍需人工合并。
.venv/bin/python -m quantlab.agent.dev_studio_cli --repo-root . --output ./artifacts \\
  --run-cycle --task-id '<实际 task_id>'
```
...
我的股票保存在 `artifacts/_home/my_stocks.json`，最多50只；仓位合计不超过100%，未填的部分视为现金。添加框会基于当天已生成的本地股票快照提供名称/规范代码候选（名称或代码片段均可）；候选仅辅助选择，点击候选只填入代码，不会自动添加。仍需点击“添加”显式确认；仓位、成本和原有校验规则不变。

### 复盘验证

在个股报告的“保存判断”里选看多、观望或看空，核对周期 1/3/5/10/20 个交易日（默认5），来源选“我的判断”或“AI 的判断”（把 AI 的结论记下来，以后比较谁更准），失效价和目标价可不填，再写一句理由。判断保存在 `artifacts/_home/judgments.json`，最多2000条。
...
AI 助手的日常模式可以用 `get_judgments` 读取这些判断和统计（含按大V作者分组）；复盘页的“问 AI 复盘”会带上统计和最近15条判断。

### 数据中心

数据中心只读[数据清单](../reference/data-catalog.md)（数据侧维护），显示全部数据集的类型（文件/数据库/查询接口/推送）、内容、覆盖、状态（可用/待审查/未就绪/已停用）和“牛牛怎么用”，可以按状态、类型筛选和搜索，单击看位置、格式和完整说明。“覆盖”一列是从清单文字里读出的截止或观察日期，不是实际检查结果。

页面上方有四个分区，对应数据侧 2026-09-25 开放的接口（`quantlab.data.data_services`，清单 §3.5）：

- **数据目录**：同上。
- **更新状态**（`DataStatusService`）：每个数据集的最新日期、最近成功更新、行数、文件数、状态（正常/落后/异常/未知）和原因、已封存到哪天；盘中记录器那几行显示今天记录了几次、失败几次。下面是最近 30 天的封存记录（版本、文件、行数、待封存的数据、核对结果）。“只看落后、异常和未知”可以只看有问题的。
- **预览与试查询**（`DataPreviewService`）：文件数据按代码或日期看前几行（默认 20 行，最多 200 行）；查询接口按数据侧给出的参数说明填好后试查一次，会真实联网，一次一查。
- **更新与封存**（`DataUpdateJobs`）：选任务（收盘后日常更新、启动/停止盘中记录器、补某一天、封存某一天、核对封存、撤销封存），填参数后先“生成计划”，看清每一步是采集、跳过、封存还是核对、会不会覆盖；计划被阻止（已封存、数据盘没连、同一任务在跑、非交易日等）时不能执行。确认后任务在数据侧的后台进程里执行，关掉牛牛也不中断；下方运行记录每 3 秒刷新，单击看进度、结果和日志，运行中可以取消。盘中记录器在交易日打开牛牛时会自动启动，这是唯一不经确认的任务。

采集、校验、写入和封存都由数据侧负责，牛牛只调用这些接口。接口在清单里不是 READY 时，对应分区按钮是灰的。

原来的“数据中心”（研究实验用过的行情快照、股票池版本、归档日线研究输入）仍在专业模式里，可从新页面底部打开；其中由牛牛直接去 Baostock 获取参考数据的按钮已移除，参考数据改由数据侧的更新任务采集。

### 盘中板块

数据来自数据侧的盘中板块接口（`quantlab.data.sector_intraday.SectorIntradayProvider`，扶摇/同花顺板块指数），只在[数据清单](../reference/data-catalog.md) §3.4 把 `sector_board_snapshot`、`sector_board_members` 标为 `READY` 时启用（2026-09-24 已开放）；若数据侧改回待审查，页面会说明原因，专业模式下有“开发预览”按钮供核对。数据侧尚未实测盘中延迟、集合竞价和全天限流（2026-09-28 实测），所以页面始终显示数据时间和离现在几秒，延迟大时一眼能看到。

- 左边是板块排行，可切换概念/行业和涨幅榜/跌幅榜/成交额；默认勾选“过滤全市场标签和小板块”，隐藏数据侧标为全市场标签（`board_class=market_label`：融资融券、沪股通、精选指数、次新股、业绩预增等）的板块，以及成分股少于 10 只的板块（成分数来自数据侧每日成分快照），取消勾选显示全部；单击一个板块，右边列出成分股（按涨跌幅排序），标出涨停、炸板、跌停、停牌，以及和腾讯/新浪核对不上而暂不显示价格的股票。双击股票打开个股报告。
- 交易时间板块榜每 10 秒、打开的板块每 5 秒刷新（数据侧规定的最小间隔，接口在间隔内返回缓存）；午休和收盘后每分钟检查一次，显示最近一次行情。页面顶部写明交易状态、数据时间、离现在几秒、刷新失败时显示的是上一次结果。
- 板块走势是页面打开后每次刷新的采样，不是分时线（数据侧没有分钟线）；成分为当前成分，不代表历史。
- “问 AI 解读”带上涨幅前后列和当前板块的成分股概况；AI 助手也可以用 `get_intraday_sectors` 自己查（接口开放时才出现）。

### 日内做T

数据只来自[数据清单](../reference/data-catalog.md) §3.6 的 `gst_intraday`（16 只股票，成交与 1 分钟K 2019-05-29 至 2024-10-24），READY 时启用；只读打开数据库，按股票和日期取 `bars_1m`、`stock_days`、`stocks`、`ticks`，不读 `*_all` 表、不改库。涨跌停价按清单规则用 `stock_days.prev_close` 推算（主板 ±10%，创业板 2020-08-24 起 ±20%、之前 ±10%，ST ±5%，ST 状态取 `security_status_baostock_v2`）。

- **底仓做T 的意思**：假设前一天收盘时手里有一笔底仓（默认 10 万元，按昨收折成整百股）。策略看空时“先卖后买”——先卖掉一部分底仓，晚些买回；看多时“先买后卖”——先多买一些，晚些卖掉同样数量的旧股。每天收盘前仓位回到原样，今天买的股票当天不卖，所以不违反 T+1。收益按“比一直拿着不动多赚或少赚多少”计算，单位是基点（1 bp = 0.01%，相对底仓市值）。
- **九种策略**：前六种是在这份数据上研究出来的（含可调参数的“趋势突破（开盘区间）”，只作对照：训练期 +18 bp、检验期 +10 bp，不显著），都在收盘集合竞价收尾；除“低开回补”是开盘先买、收盘卖旧股、“趋势突破”可设方向外，其余只做先卖后买。**上午打分（逐年滚动）**是目前最好的：10:00、10:30 用 10 个指标预测到收盘的涨跌，每年只用之前年份拟合的系数，2023–2024 年每笔扣费后约 +36 个基点、胜率 62%、t≈2.1，动用一半底仓时对底仓年化约 +3%（全部底仓约 +6%），但 2023 年基本持平。全天打分（加上下午三个时点）、尾盘弱势、尾盘打分较弱；低开回补去掉 2024 年初急跌后不赚钱，只作对照。研究过程见[日内做T 策略研究](../archive/intraday/20260925-日内做T策略研究.md)。后三种来自用户提供的文章《日内交易策略——当天收盘前必须走人》，按底仓做T 改写：开盘突破（09:30–10:00 的高低点，突破做多、跌破做空，止损在区间中点）；VWAP 回归（偏离当天成交量加权均价超过 2 倍 60 分钟标准差时反向做，回到 VWAP 平仓，止损为最近 20 分钟高低差的 0.5 倍）；尾盘动量（14:30 时下午涨跌超过 1% 且 14:00–14:30 放量就跟随，14:56 平仓）。参数可在页面上改。文章的风控一并采用：开盘前 5 分钟不交易，只在 09:35–11:00 和 13:30–14:50 开仓，14:50 起强制平仓，当天亏损达底仓 1% 停止，每天最多 20 笔。文章三种策略扣费后都稳定亏损。
- **成交假设**（偏保守）：信号在一根 1 分钟K 收完后产生，下一根开盘价成交并按滑点吃亏（默认每边 1 个价位 0.01 元，依据盘口实测的买卖价差；也可选 0.1%）；尾盘两种研究策略在收盘集合竞价按收盘价买回；每分钟最多成交该分钟成交量的 20%，按整百股；整分钟都在涨停价成交时买不进、在跌停价成交时卖不出；开仓单 3 分钟没成交就撤，平仓单一直挂到成交；到收盘还没回补的部分按收盘价估值并单独计为“未能回补”。费用：佣金（默认万 2.5、最低 5 元，可选万 1 免五）、过户费万 0.1、卖出印花税（2023-08-28 前 0.1%，之后 0.05%）；也可选“不计费用和滑点”只看信号本身。
- **结果怎么看**：默认 2022-12-31 及以前为训练期、之后为检验期——调参数只看训练期，最后用检验期判断是否只是碰巧。页面给出每天平均收益、t 值、每笔扣费后和不计成本的收益、胜率、最大回撤，以及按年份和按股票的拆分、累计收益曲线和全部交易明细。每次回测自动保存在输出目录 `_intraday/runs/`，可从“已保存的回测”重新打开。
- **分时回放**：选股票和日期看当天 1 分钟走势、均价线、昨收、涨跌停价和主动买卖量；在交易明细里双击一笔交易会直接打开那一天并标出买卖点；“用上面设置的策略模拟这一天”可以对任意一天单独试。
- **限制**：只有这 16 只股票，结论不能推广到全市场；ticks 是约 3 秒一次的成交汇总，不是逐笔，无法模拟排队；五档盘口只有 2019-05 至 2020-06，回测没有使用盘口，统一用成交价加滑点。文章三种策略全量实测在训练期和检验期都是扣费后稳定亏损，不计成本时每笔平均只有 −1 到 +3.5 bp，远小于约 20–35 bp 的来回成本；“大盘”在这里是 16 只股票的平均，不是指数。这是研究工具，不是买卖建议。

### 大V复盘

1. 在“添加复盘文章”里填作者、平台、发布日期，粘贴正文（可填标题和原文链接），保存。文章存在 `artifacts/_home/kol/posts.json`，最多1000篇、每篇3万字。牛牛不登录、不订阅、不抓取任何平台；自动采集属于数据侧，数据侧交付后再接入。
2. 点“AI 提炼观点”：用模型设置里选的模型（默认 Codex；用 API 时需在环境变量里提供密钥）把正文整理成固定格式的观点——类型（大盘/个股/方向）、对象、看多/观望/看空、周期（1/3/5/10/20个交易日）和一句原话。原话在正文里找不到的标“未在原文找到”，格式不对的直接丢弃并注明条数。提炼不调用任何工具，正文按外部数据处理。也可以不用 AI，手动添加或删除观点。
3. 核对无误后点“保存为待核对”：大盘和个股观点变成来源为“大V观点”、带作者名的判断，交给复盘验证核对。起点是发布日当天或之前最后一个交易日的收盘（晚上发的文章从当天收盘算，周末发的从周五收盘算）；大盘观点按全市场等权平均涨跌核对（没有超额），个股观点与其他判断同一口径。方向观点保存在文章里，但不自动核对，因为题材和板块没有公认的成分名单。同一篇文章再次保存会替换之前的判断。
4. 页面下方“作者表现”按作者给出已完成条数、准确率、平均顺向收益和超额。“问 AI 汇总最新观点”把最近一天的文章和观点、今日市场概况一起填进 AI 助手，让它给出共识、分歧、与数据矛盾之处和明天要核对的条件。
