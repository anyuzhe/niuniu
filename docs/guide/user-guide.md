...
| 日内做T | 手里有底仓时，当天高抛低吸能不能赚钱：用 16 只股票 2019–2024 年的 1 分钟数据回测开盘突破、VWAP 回归、尾盘动量三种日内策略（扣佣金、印花税、滑点，分训练期/检验期），并逐日回放分时走势和每笔模拟交易 | 已上线（数据侧 `gst_intraday`，只有 16 只股票，研究用） |
...
- **三种策略**（来自用户提供的文章《日内交易策略——当天收盘前必须走人》，按底仓做T 改写）：开盘突破（09:30–10:00 的高低点，突破做多、跌破做空，止损在区间中点）；VWAP 回归（偏离当天成交量加权均价超过 2 倍 60 分钟标准差时反向做，回到 VWAP 平仓，止损为最近 20 分钟高低差的 0.5 倍）；尾盘动量（14:30 时下午涨跌超过 1% 且 14:00–14:30 放量就跟随，14:56 平仓）。参数可在页面上改。文章的风控一并采用：开盘前 5 分钟不交易，只在 09:35–11:00 和 13:30–14:50 开仓，14:50 起强制平仓，当天亏损达底仓 1% 停止，每天最多 20 笔。
- **成交假设**（偏保守）：信号在一根 1 分钟K 收完后产生，下一根开盘价成交并按滑点吃亏（默认 0.1%）；每分钟最多成交该分钟成交量的 20%，按整百股；整分钟都在涨停价成交时买不进、在跌停价成交时卖不出；开仓单 3 分钟没成交就撤，平仓单一直挂到成交；到收盘还没回补的部分按收盘价估值并单独计为“未能回补”。费用：佣金（默认万 2.5、最低 5 元，可选万 1 免五）、过户费万 0.1、卖出印花税（2023-08-28 前 0.1%，之后 0.05%）；也可选“不计费用和滑点”只看信号本身。
...
- **限制**：只有这 16 只股票，结论不能推广到全市场；ticks 是约 3 秒一次的成交汇总，不是逐笔，无法模拟排队；五档盘口只有 2019-05 至 2020-06，回测没有使用盘口，统一用成交价加滑点。2026-09-25 首次全量实测（普通佣金、0.1% 滑点）三种策略在训练期和检验期都是扣费后稳定亏损，不计成本时每笔平均只有 −1 到 +3.5 bp，远小于约 35 bp 的来回成本——详见[开发总档案](../project/changelog.md)。这是研究工具，不是买卖建议。
