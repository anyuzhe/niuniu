...
来源说明仅保存在本次工作台内存，便于核对，不写入v1策略包或新实验的持久父子谱系；关闭后需要重新从归档选择来源。另行导入普通包会清除该来源，编辑当前完整配置树则保留来源。另存仍只创建新的普通v1 JSON，不覆盖历史包。回填原审批面板后须重新保存待批准提案，清除旧勾选并重新批准、冻结本次实际输入；不继承旧授权、预算、任务或收益结论，也不连接Paper/实盘。本入口只面向宿主UI，没有新增模型工具权限。
