...
“结果对照”填写本工作空间内两个已完成策略包实验的完整 run_id。后端只读取这两个实际归档和其信号子实验，不回读源数据目录或重跑因子/回测；先核对身份、全部冻结输入（包括高周期背景）、股票池原始元信息及版本、运行环境和可比口径，再按已保存净值曲线校验描述性统计。净值日期须唯一并与行情对齐，成交费用缺项不能当零。不可比时保留原因而不产生数值差值，损坏或缺失归档明确失败；即便可比也不给赢家、显著性或Alpha认证。
