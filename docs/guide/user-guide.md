...
| 日内做T | 手里有底仓时，当天高抛低吸能不能赚钱：用 16 只股票 2019–2024 年的 1 分钟数据回测研究所得的两种尾盘先卖后买策略和文章里的三种日内策略（扣佣金、印花税、滑点，分训练期/检验期），并逐日回放分时走势和每笔模拟交易 | 已上线（数据侧 `gst_intraday`，只有 16 只股票，研究用） |
...
- **五种策略**：前两种是在这份数据上研究出来的“尾盘弱势”和“尾盘打分”——14:00 时个股明显走弱且 16 只股票平均也在跌（或 7 个指标的打分预计继续跌），就先卖一部分底仓，收盘集合竞价买回；它们在没有参与筛选的检验期（2023-01 至 2024-10）扣费后每笔约 +28 / +38 个基点，但交易少，对整笔底仓每年只贡献约 1%，统计证据中等（t≈1.6–2.1），研究过程见[日内做T 策略研究](../archive/intraday/20260925-日内做T策略研究.md)。后三种来自用户提供的文章《日内交易策略——当天收盘前必须走人》，按底仓做T 改写：开盘突破（09:30–10:00 的高低点，突破做多、跌破做空，止损在区间中点）；VWAP 回归（偏离当天成交量加权均价超过 2 倍 60 分钟标准差时反向做，回到 VWAP 平仓，止损为最近 20 分钟高低差的 0.5 倍）；尾盘动量（14:30 时下午涨跌超过 1% 且 14:00–14:30 放量就跟随，14:56 平仓）。参数可在页面上改。文章的风控一并采用：开盘前 5 分钟不交易，只在 09:35–11:00 和 13:30–14:50 开仓，14:50 起强制平仓，当天亏损达底仓 1% 停止，每天最多 20 笔。文章三种策略扣费后都稳定亏损。
