...
- 左边是板块排行，可切换概念/行业和涨幅榜/跌幅榜/成交额；默认勾选“过滤全市场标签和小板块”，隐藏融资融券、沪股通、同花顺指数、次新股、业绩预增这类不代表题材的标签，以及成分股少于 10 只的板块（只数要等数据侧提供，之前只按名称过滤），取消勾选显示全部；单击一个板块，右边列出成分股（按涨跌幅排序），标出涨停、炸板、跌停、停牌，以及和腾讯/新浪核对不上而暂不显示价格的股票。双击股票打开个股报告。
