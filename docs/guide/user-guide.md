...
启用左下角“专业模式”，进入“开发工作台”，点击“六角色模型”设置 LEAD（总控）、DATA（数据）、CORE（研究交易）、AI（助手运行时）、APP（产品界面）、QA（独立测试审核）。角色可以使用相同模型；支持现有 Codex CLI、Responses、Chat Completions 协议。默认继承当前模型设置，之后可逐角色修改模型 ID、推理强度、地址、超时、上下文与工具预算；密钥只填环境变量名，不填写真实密钥。保存配置不会调用模型。
