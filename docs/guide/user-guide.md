...
| 复盘验证 | 在个股报告里保存的判断（看多/观望/看空、5/10/20个交易日、可设失效价和目标价），按之后的真实走势自动核对，统计准确率、顺向收益和相对全市场的超额，并分“我的判断/AI 的判断”对比 | 已上线 |
...
在个股报告的“保存判断”里选看多、观望或看空，核对周期 5/10/20 个交易日，来源选“我的判断”或“AI 的判断”（把 AI 的结论记下来，以后比较谁更准），失效价和目标价可不填，再写一句理由。判断保存在 `artifacts/_home/judgments.json`，最多2000条。

核对由 [judgments.py](../../src/quantlab/trading/judgments.py) 完成，只读 DATA `READY` 的前复权日线和当天市场总览里的“全市场等权平均日收益”序列：

- 收益从判断日收盘算到第 N 个交易日收盘（前复权，分红送转不算涨跌）；停牌日照样计入交易日。
- 失效价、目标价按每天的实际（不复权）收盘价核对，和报告上看到的价格一致；先到哪个就在那天结束：到失效价算判断错误，到目标价算正确。
- 都没到的，到期看顺向收益（看多按涨跌、看空按涨跌取反）：超过 +1% 为正确，低于 −1% 为错误，其间为基本持平；观望只记录走势，不计对错。
- 同时给出同期全市场等权平均和超额。准确率只按涨跌判断，不看是否跑赢市场，所以表里把超额单独列出。
- 核对完的结果写回记录，以后数据更新或市场序列窗口变化都不会改动历史结论；已完成的看多/看空判断少于10条时页面会提示参考意义有限。

AI 助手的日常模式可以用 `get_judgments` 读取这些判断和统计；复盘页的“问 AI 复盘”会带上统计和最近15条判断。
