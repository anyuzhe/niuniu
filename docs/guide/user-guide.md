...
| 数据中心 | 牛牛能用哪些数据：数据清单里全部文件、数据库、查询接口的内容、覆盖日期、状态和用法，可搜索筛选；更新状态、预览和在页面上发起更新/归档等数据侧接口开放后接入 | 数据目录已上线 |
...
每个数据集的真实更新状态、数据预览与接口试查询、在页面上发起更新和归档，都需要数据侧提供接口（需求见 Project 文档 `claude/niuniu-data-request-data-center.md`，清单 ID 建议为 `data_status_service`、`data_preview_service`、`data_update_jobs`）；接口在清单里标为 READY 前，页面写明“数据侧尚未提供”。采集、校验、写入和归档始终由数据侧负责，牛牛只调用接口，执行更新前先显示计划并确认。

原来的“数据中心”（研究实验用过的行情快照、股票池版本、归档日线研究输入）仍在专业模式里，可从新页面底部打开。
