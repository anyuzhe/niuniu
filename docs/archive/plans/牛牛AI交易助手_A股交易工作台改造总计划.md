...
### P8.6-C Research Skill Library / Agent Read-only Retrieval（v3 已完成，2026-09-16）

- `research_skills/library.json` 作为Git-first宿主授权表，同时固定 control/archive/package snapshot 与 curation plan；正式读取要求 `research_skills/` Git-clean。
- 每次读取重新审计archive receipt及全部对象、策展包资源和plan推导lineage；不接受 `latest`、任意路径或未注册包。
- Research Lab新增只读技能库；日常AI研究助手、AI Team Peer Review与标准MCP共用4个严格Schema工具，支持包摘要、claims/hypotheses/alignment/resources检索和来源定位。
- 来源片段最多6000 UTF-8 bytes且offset必须位于字符边界；`SCRIPT`拒绝展示，正文固定标为不可信外部数据而非命令。
- Library无clone/fetch、脚本执行、StrategySource/Playbook写入、Decision/Paper或订单接口；郑希包仍保留publication/source authenticity blockers与全部非资格边界。
...
### P13 Paper → Real 的渐进式交易层（P13-A / P13-B0 已完成；P13-B1+ 需通道与单独评审）
...
- **P13-A 已完成**：新增 `ReadOnlyBrokerAdapter`、脱敏 `BrokerSnapshotStore`、`BrokerShadowReconciler`、`niuniu-broker-shadow` 与 MCP `get_broker_shadow`。只接受无凭证的 CNY 账户快照，宿主导入需显式确认；持仓/现金与 Dynamic Paper 做 Shadow 对账，权益差仅描述。专项7/7、Broker+Health+MCP 24/24、完整仓库 **889/0/0**。
- **P13-B0 已完成**：新增 Broker capability registry、禁用态 `RealTrade policy`、`RealTradeReadinessService`、`niuniu-real-trade-readiness`、MCP `get_real_trade_readiness` 与 System Health 观察项。当前只有 `json-export-v1` offline Adapter，故无通道时固定 `NO_LIVE_BROKER_CHANNEL` + BLOCKED。新增8项、联合32项通过，完整仓库 **897/0/0**。
- B0 policy 不猜用户限额，示例中的单笔/单票/总敞口/日损/订单数/快照新鲜度保持 null；`enabled=true` 或关闭人工确认/kill switch/Shadow/Strict PIT/System Health 会直接拒绝。
- **P13-B1**：只有出现明确可用的具体券商实时只读通道后才开始；目标仍是账户/持仓/资金/回执只读连接，不默认带订单权限。
...
- 权威规划与进度：[牛牛AI交易助手_自主研究与打板情绪研究_规划与进度.md](../autonomous-research/牛牛AI交易助手_自主研究与打板情绪研究_规划与进度.md)。
...
3. **Strict PIT Evidence/Coverage、PIT Universe v1 与连续 SecurityStatus v2 工程已完成；真实官方资料继续补齐（当前内部主线）**：Universe、status、industry、daily market cap 均需权威 URL、本地官方原文 SHA256 与确认 publication time。SecurityStatus v2 必须绑定 exact-session Universe、覆盖其全部 members，并只通过显式相邻 snapshot link 证明进入/持续/撤销；Coverage 只做 inventory，不生成总完成率。真实 Universe/v2 status snapshot 均为0，现有状态资料仍只是14条/7只股票的稀疏 statement；另有1个 MarketRules v2 snapshot/7个停牌 session。2026-09-16 已为 2026-09-17 保存三所5,563只review基线和规范语义 addendum，但目标日刷新及三项确认均未完成；SZSE公开批量完整状态源、BSE公开字段权威映射仍阻塞。目标日上午原需人工刷新；现已按用户授权安装一次性fail-closed LaunchAgent，于08:00启动并在09:10安全停止：先Universe，只有三所状态全部闭合才建SecurityStatus；任一确认门不成立即保持receipt=0，错过不得补档。
4. **个股问答自动实时行情（已完成 v1，2026-09-16）**：用户当前轮明确股票代码/正式名称即授权一次、最多10只的腾讯/东财/新浪只读共识查询；唯一股票上下文追问可沿用，多股歧义不猜。结果在模型推理前注入，但不写正式MarketSnapshot、Decision、信号或订单。
5. **Watch 序贯/在线衰减统计（已完成 v1，2026-09-15）**：新 Watch 冻结 family alpha、最小 Rank IC 衰减、最少新增成熟日期与非重叠 block；新增成熟样本进入 mixture e-process，legacy Watch 不静默迁移，越界只进入人工复核。
6. **真实前瞻每日样本（持续运行）**：2026-09-15 已接受首个当前工作空间 DailyMarket day，并为 2026-09-16 冻结 `EXTREME_RISK / NO_TRADE` PREP；空 CandidateSet 以 `COMPLETE_NO_TRADE` 正常闭环，不制造 Paper 样本。后续按交易日持续积累，非空候选才进入 AUCTION/R1/R2/R3。
7. 跨 workspace 研究记忆与用户研究偏好。
...
- 2026-09-12：P1 + P2 基础完成并通过验收。新 8 个业务一级入口已落地，旧 12 个研究页保留兼容；Decision Ledger 已具备 append-only、revision、幂等、checksum、人工桌面创建/修订和股票检索。最终全仓 688 项通过，端到端验收创建研究任务 0。阶段说明见 `docs/archive/trading-workbench/牛牛AI交易工作台_P1P2新导航与DecisionLedger_验收说明.md`。下一阶段：P3 Stock Dossier。
- 2026-09-12：P3 Stock Dossier 完成。股票中心已可按证券聚合当前/历史 Decision、精确关联实验、Factor Watch，并可反向打开原实验/跟踪证据；打开档案不创建研究任务。最终全仓 692 项通过，端到端聚合新增研究任务 0。阶段说明见 `docs/archive/trading-workbench/牛牛AI交易工作台_P3StockDossier_验收说明.md`。下一阶段：P4 A 股主线市场 / Theme Matrix。
- 2026-09-12：P4 A 股主线市场 / Theme Matrix 完成。Theme Snapshot 已支持 append-only revision、市场事实来源/时点约束、Machine/AI 状态分离、Decision/量化证据关联与只读模型工具；缺失格子保持 UNKNOWN，不从标签或 AI 文本推导事实。最终全仓 700 项通过，隔离端到端验收新增研究任务 0。阶段说明见 `docs/archive/trading-workbench/牛牛AI交易工作台_P4主线市场ThemeMatrix_验收说明.md`。下一阶段：P5 Decision Frame。
- 2026-09-13：P5 Decision Frame 完成。PREP/AUCTION/R1/R2/R3/D1/D2/D3+ 已升级为可审计时间合同；真实提交时间、Frame Policy 版本、EARLY/ON_TIME/LATE/BACKFILL、跨轮 missing/变化与 D1+ 原判关联均正式落盘，旧 Decision 保持可读。最终全仓 710 项通过，隔离端到端新增研究任务 0。阶段说明见 `docs/archive/trading-workbench/牛牛AI交易工作台_P5DecisionFrame_验收说明.md`。下一阶段：P6 策略动作状态机。

- 2026-09-13：P6 策略动作状态机完成。Strategy Intent 已正式约束 DISCOVERED/WATCH/READY/PLAN_OPEN/OPEN/ADD/HOLD/REDUCE/EXIT 与 INVALIDATED/REJECTED/EXPIRED；非法跳级、同 Frame 重复当前状态、历史插入和后续状态下改写旧动作均被阻断；当前状态按 trading_day + Frame 业务时间计算，策略意图继续与 Paper/真实成交严格分离。最终全仓 720 项通过，隔离端到端新增研究任务 0、模型写状态工具 0。阶段说明见 `docs/archive/trading-workbench/牛牛AI交易工作台_P6策略动作状态机_验收说明.md`。下一阶段：P7 今日交易驾驶舱。

- 2026-09-13：P7 今日交易驾驶舱完成。默认首页已统一聚合 Strategy Intent、Theme Snapshot 正式 facts、已保存 AI Thesis、Risk Review、Research Agenda 与 Watch；默认日期取工作空间最新有证据的业务日，不按自然时钟猜盘中 Frame；首页只读，不调用模型、不刷新 Watch、不下载或执行研究。最终全仓 724 项通过，隔离端到端新增研究任务 0、Watch/失败任务归档前后未改。阶段说明见 `docs/archive/trading-workbench/牛牛AI交易工作台_P7今日交易驾驶舱_验收说明.md`。下一阶段：P8 AI Team / Peer Review。
...
P13-B0 已完成无通道条件下的 RealTrade fail-closed 安全门后，正式产品路线只剩具体券商通道及其后续权限边界：
...
- 使用 9/14 实际行情 BACKFILL 重放仍选择超声电子；全仓 **769 tests / 0 failed / 0 skipped**。阶段说明见 `docs/archive/trading-workbench/牛牛AI交易工作台_P8_5D实时快照与DailyScanner_验收说明.md`。
...
- 2026-09-15：Strict PIT SecurityStatus 第二批真实证据完成。新增东旭蓝天、易事特、得润电子、合力泰4份深交所官方公告，按官方精确 `publishTime` 和本地 PDF SHA256 归档8条停牌/复牌及ST生效 receipt；真实 `niuniu-data` 累计7只证券、14条 verified receipt，silver 表重新物化为14行。深度审计14/14有效，Coverage 对这7只证券显示 SecurityStatus evidence presence，但其它三类仍为0；PREP真实烟测命中2条新状态证据并因缺MarketRules/完整状态链/PIT Universe保持PARTIAL。专项20/20、完整仓库961/0/0；详情见《牛牛AI交易工作台_StrictPITSecurityStatus第二批验收说明.md》。下一步继续补同证券状态链并建设官方逐日MarketRules receipt。
- 2026-09-15：首轮当前工作空间 Daily Orchestrator 前瞻 NO_TRADE 完成。接受 2026-09-15 Baostock 全市场日快照 5,219 行并于合法 PREP 窗口冻结 2026-09-16 `SYSTEM_PREDICTION`；工程 Router 输出 `EXTREME_RISK / NO_TRADE`，CandidateSet 为 `PARTIAL / RETROSPECTIVE_REFERENCE` 且为空。补齐 `COMPLETE_NO_TRADE` 终态和后续 Frame `SKIPPED_NO_TRADE`，只写 NO_TRADE bridge receipt，不请求无标的实时行情、不制造 Decision/PaperPlan/成交；终态重复 tick 文件哈希和 mtime 不变。专项38/38、完整仓库962/0/0；详情见《牛牛AI交易工作台_首轮DailyOrchestrator前瞻NoTrade_验收说明.md》。下一步继续逐日运行，并行建设官方逐日 MarketRules/PIT Universe/状态链。
...
- 2026-09-16：完成External Research Skill v3只读Library。新增Git-clean `library.json`授权、control/archive/package/plan四重身份与lineage复核、6000-byte UTF-8有限片段、安全Agent工具及Research Lab入口；日常助手、Peer Review和MCP可按精确snapshot检索郑希10 claims/5 hypotheses/alignment/locator。外部正文固定为不可信数据，SCRIPT拒绝，0网络/0执行/0结构化写入；Library新增专项6/6，完整仓库982/0/0，真实审计结果见独立验收说明。
- 2026-09-16：完成 PIT Universe Receipt v1 工程合同。新增离线 `pit-universe-archive/audit`，按 effective session append-only 固定声明 scope、全部成员、source↔exchange、官方原文字节/SHA256及 `published_at/available_at/created_at/cutoff_at`，cutoff 后首次创建、篡改、子集扫描和调用方自报 verified 均 fail-closed。Qualification 要求每个 bar session 唯一 receipt；approval freeze 保存实际 membership；PREP/Orchestrator 精确绑定目标 trading day snapshot；Coverage/System Health 展示全局 inventory 但不冒充 case coverage。真实数据根仍为0个 snapshot，本轮未联网；专项46/46、完整仓库990/0/0。
- 2026-09-16：完成连续 SecurityStatus v2 工程合同。新增全Universe逐日 `TRADABILITY + RISK_WARNING` receipt、PIT Universe exact-session绑定、三项宿主确认、09:15 cutoff、内容寻址原文、append-only深验与显式 previous snapshot 链；root、Universe进出、同日歧义、链分叉和`UNKNOWN`均按边界 fail-closed。silver同时兼容稀疏statement和完整v2，PREP只消费exact session并标记`STRICT_PIT_DAILY_COVERAGE`；Coverage/System Health增加完整状态inventory。真实数据只读审计仍为0个v2 receipt/0条连续link，本轮未联网、未回填历史；聚焦集成46/46，完整仓库1000/0/0（非Desktop879 + Desktop121）。详情见《牛牛AI交易工作台_连续SecurityStatusV2_验收说明.md》。
- 2026-09-16：完成 2026-09-17 前瞻 Universe / SecurityStatus 取证准备。独立 staging 父 capture 固定 SSE 2,318、SZSE 2,901、BSE 344，共5,563只 review-only A_SHARE baseline；追加式 addendum 保存8份官方规范/通知、3次SZSE站内检索和10次官方静态域名有界路径响应。SSE规范语义较完整；SZSE规范只证明FTS私有日文件与状态码，公开批量源排查未命中；BSE规范只证明FDEP文件，公开字段映射未闭合。publication time、公共字段语义映射、目标日Universe/状态完整性均未确认，archive未调用，正式Universe/v2 status receipt继续为0。后续已获一次性条件式无人值守授权：LaunchAgent于目标日08:00启动，08:05/08:15作进程恢复，09:10安全停止；先固定目标日staging，三项机器门全通过才归档，09:15硬截止且不得历史补档。详情见《牛牛AI交易工作台_20260917前瞻Universe与SecurityStatus取证阶段验收说明.md》。
- 2026-09-16：完成个股问答自动实时行情v1。明确A股代码或本地正式名称的当前问题，视为该轮明确证券的一次只读联网授权；宿主在模型推理前执行三源共识并注入价格/时点/市场状态。支持唯一股票上下文追问，单轮上限10只，多股歧义和两源不一致均fail-closed。临时报价只进聊天证据，不写MarketSnapshotStore、Decision、Theme、Playbook或订单；正式交易链的Frame/PIT/规则门不变。详情见《牛牛AI交易工作台_个股问答自动实时行情V1_验收说明.md》。
