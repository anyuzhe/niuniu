...
| Data and universes | Read-only MQC Parquet access, raw/qfq prices, data quality audits, fixed data versions, historical listing intervals, and PIT Universe v1 receipts that bind one effective session's complete declared scope and members to official exchange bytes, SHA256, publication/availability times, and a pre-09:15 cutoff |
| Data qualification | Request-scoped `research_only / retrospective_reference / strict_pit / official_rule_covered`; sparse eligibility/status statements or caller booleans cannot certify completeness. PIT Universe v1 binds exact-session members; SecurityStatus v2 binds daily tradability/risk-warning for every member and derives enter/continue/clear only through explicit adjacent-session links |
...
| Stock-question live quotes | Naming an A-share code or exact local `stock_basic` name in the current chat turn authorizes one bounded, read-only Tencent/Eastmoney/Sina consensus lookup for at most ten explicit stocks. The host injects price, timestamp, and market status before model inference; it does not create a formal MarketSnapshot, Decision, signal, or order |
...
- Ad-hoc stock questions now trigger a turn-scoped read-only public-web consensus quote when the stock is explicit. This improves informational answers but remains non-persistent, non-Strict-PIT, and without an exchange/broker SLA; it cannot by itself justify a trading signal.
- A read-only staging capture for the prospective 2026-09-17 session now holds a 5,563-member review baseline (SSE 2,318; SZSE 2,901; BSE 344), official technical specifications, and bounded SZSE public-source diagnostics. It is not target-session evidence: publication times, public-field mappings, and complete daily status remain unconfirmed, so both formal receipt counts remain zero. The user subsequently authorized one fail-closed unattended run: LaunchAgent `com.niuniu.pit-20260917` starts at 08:00 Asia/Shanghai (08:05/08:15 are process-recovery triggers), stages official target-day bytes, and stops safely at 09:10. It archives only if every machine-verifiable confirmation gate passes; otherwise receipts remain zero, and nothing is backfilled after 09:15.
