...
root=Path('artifacts/chan-500-full-ten-year-qfq');job=json.loads((root/'runs/_jobs/8a49ef3e-f46b-4d4d-8508-7ec3439bcab3.json').read_text())
...
assert bars['symbol'].n_unique()==500;assert bars.height==1214000
assert bars.group_by('symbol').len()['len'].unique().to_list()==[2428]
assert record['manifest']['data_snapshot']['adjustment']=='raw'
assert record['manifest']['signal_data_snapshot']['adjustment']=='qfq'
signal_bars=pl.read_parquet(source/'bars.parquet')
assert bars.select('symbol','datetime','available_at').equals(signal_bars.select('symbol','datetime','available_at'))
previous=json.loads(Path('artifacts/chan-500-ten-year/sample-manifest.json').read_text())
assert not set(previous['symbols']) & set(sample['symbols'])
