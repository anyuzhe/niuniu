"""Frozen historical Brooks wedge reversal workflow acceptance."""
import argparse
import json
...
if __name__=='__main__':
    ...
    (args.output/'fixed-plan.json').write_text(encode({'request':request,'plan':plan,'parameters':'registered defaults',
        'scope':'Confirmed-pivot contraction reversal proxy; no intrabar fill or statistical-power claim'}))
    ...
    for direction in (1,-1):
        factor=BrooksWedgeComponent('confirmed',direction)
        ...
        for component in COMPONENTS:
            assert_prefix_invariant(BrooksWedgeComponent(component,direction),market,{},checks)
        for cutoff in checks:
            _,before_states,before_events=factor.trace(market.filter(pl.col('available_at')<=cutoff),{})
            if before_states!=[s for s in direction_states if s.available_at<=cutoff] or before_events!=[e for e in direction_events if e.available_at<=cutoff]:raise ValueError('Wedge history changed')
        values.write_parquet(args.output/('confirmed-up.parquet' if direction==1 else 'confirmed-down.parquet'))
    ...
    params,origin=resolve_template('RESEARCH.BROOKS_WEDGE_REVERSAL',runner.registry)
    config=ExperimentConfig('Brooks 三极值收缩反转全流程验收',request,'COMB.CONDITION',parameters=params,horizons=(1,5,20),theory_origin=origin,replay=True,sequence_audit=True)
    ...
    summary={'status':record['status'],'artifact_path':result.artifact_path,'rows':market.height,'symbols':len(symbols),'data_snapshot':batch.snapshot,
        'counts':counts,'episode_end_states':dict(Counter(last_status.values())),'prefix_cutoffs':3,'factor_prefix_comparisons':24,'event_transition_prefix_comparisons':6,'replay_cursor_checks':replay_checks,'children':len(record['children'])}
