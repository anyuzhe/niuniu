"""Frozen historical Brooks breakout/pullback workflow acceptance."""
import argparse
import json
...
if __name__=='__main__':
    ...
    plan={'split':{'train_end':'2026-08-14','valid_end':'2026-08-21'},
        'schedule':{'train_days':12,'valid_days':7,'test_days':14},'input':'entry','grid':{'max_bars':[20,40]}}
    (args.output/'fixed-plan.json').write_text(encode({'request':request,'plan':plan,'parameters':'registered defaults',
        'scope':'Close-confirmed breakout/pullback proxy; no intrabar fill or statistical-power claim'}))
    ...
    for direction in (1,-1):
        factor=BrooksBreakoutComponent('resumed',direction)
        values,direction_states,direction_events=factor.trace(market,{})
        # Check every registered component at the same three real-data prefixes.
        for component in COMPONENTS:
            assert_prefix_invariant(BrooksBreakoutComponent(component,direction),market,{},checks)
        for cutoff in checks:
            _,before_states,before_events=factor.trace(market.filter(pl.col('available_at')<=cutoff),{})
            if before_states!=[s for s in direction_states if s.available_at<=cutoff] or before_events!=[e for e in direction_events if e.available_at<=cutoff]:raise ValueError('Breakout history changed')
        values.write_parquet(args.output/('resumed-up.parquet' if direction==1 else 'resumed-down.parquet'))
        states.extend(direction_states);events.extend(direction_events)
    ...
    (args.output/'events.json').write_text(encode(events));(args.output/'transitions.json').write_text(encode(states))
    params,origin=resolve_template('RESEARCH.BROOKS_BREAKOUT_PULLBACK',runner.registry)
    config=ExperimentConfig('Brooks 突破回踩全流程验收',request,'COMB.CONDITION',parameters=params,horizons=(1,5,20),theory_origin=origin,replay=True,sequence_audit=True)
    ...
    summary={'status':record['status'],'artifact_path':result.artifact_path,'rows':market.height,'symbols':len(symbols),'data_snapshot':batch.snapshot,
        'counts':counts,'episode_end_states':dict(Counter(last_status.values())),'prefix_cutoffs':3,'factor_prefix_comparisons':42,'event_transition_prefix_comparisons':6,'replay_cursor_checks':replay_checks,'children':len(record['children'])}
