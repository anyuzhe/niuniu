...
- 当前稳定基线：P8.5-E1 已完成，全仓 `784 passed / 0 failed / 0 skipped`
...
| `ExpertSource` 作为交易者来源 | 已完成；待兼容抽象为通用 StrategySource |
| PREP 全市场扫描 / MarketSnapshot / AUCTION / R1 Scanner | 已完成 |
| DailyMarket 全市场日增量归档 | 已完成 |
| 真实前瞻冻结与防历史回填 | 已完成并已启动样本积累 |
| 通用 StrategySource 多来源对象 | 尚未正式实现 |
| 受控每日自动编排 PREP→AUCTION→R1 | 尚未完成 |
...
当前生产代码最近完整回归基线：**784 tests / 0 failed / 0 skipped**。
## 12. 后续开发主线

新版路线按“先让知识来源通用化，再让每日闭环自动运行”的顺序推进：

1. **P8.6 Trading Knowledge / StrategySource**：在不破坏 ExpertSource 历史对象的前提下增加通用来源模型，支持 TRADER / USER_EXPERIENCE / PUBLIC_METHOD / HISTORICAL_CASE / STATISTICAL_DISCOVERY / SYSTEM_REVIEW。
2. **P8.7 Daily Orchestrator**：收盘后 DailyMarket capture → 数据就绪 → PREP → 09:25 AUCTION → 09:35 R1 → R2/R3 的受控每日编排。
3. **P8.8 Playbook → Trading Desk → Paper 接线**：把前瞻选择转成 Decision / Strategy Intent，并进入长期 Paper，而不是自动实盘。
4. **P9 Agent Scorecard**：在有足够真实前瞻 Decision 后评价不同任务类型，不做模型总排行榜，也不自动调权。
5. **P10 Dev Studio + Dynamic Agent Orchestrator**：Main Agent、动态 Subagents、隔离 worktree、path lease、Tester、Reviewer、Human Merge。
6. **P11 System Health → P12 移动端 → P13 Paper→Real**。
