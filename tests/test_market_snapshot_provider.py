...
class MarketSnapshotProviderTests(unittest.TestCase):
    ...
    def test_cli_and_mcp_status_are_read_only(self):
        ...
        reply=api.call('get_market_snapshot_provider_status',{});self.assertTrue(reply['ok']);self.assertEqual(reply['data']['status'],'READY')
    ...
    def test_system_health_surfaces_provider_gap_without_marking_research_broken(self):
        ...
        self.assertEqual(provider['status'],'OK');self.assertTrue(provider['evidence']['live_provider_available'])
