...
class SupervisorTests(unittest.TestCase):
    def config(self):return {'machine_name':'601','code_root':'D:/AI/testData/niuniu','data_root':'D:/AI/testData/niuniu-data','control_root':'D:/AI/testData/tdx-control','identity_file':'C:/Users/PC/.ssh/id_ed25519','known_hosts_file':'D:/AI/ollama-tunnel/known_hosts','cluster_id':'a'*64,'shard_id':2,'allow_tunnel_start':True}
    ...
    def test_homepc_platform_denial_not_bypassed(self):
        c={**self.config(),'machine_name':'homepc','shard_id':1}
    ...
    def test_ssh_uses_only_scoped_loopback_forward_and_pinned_host(self):
        cmd=s.tunnel_command(self.config(),'C:/Windows')
    ...
    def test_worker_cannot_start_prepare_or_clear_stop(self):
        cmd=s.worker_command(self.config());self.assertIn('quantlab.agent.tdx_worker_service',cmd)
        for forbidden in ('prepare','resume','--canonical-root'):self.assertNotIn(forbidden,cmd)
