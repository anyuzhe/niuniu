...
from quantlab._vendor.chanpy.Bi.Bi import CBi
from quantlab._vendor.chanpy.Common.CEnum import BI_DIR, MACD_ALGO, TREND_LINE_SIDE
from quantlab._vendor.chanpy.Common.ChanException import CChanException, ErrCode
from quantlab._vendor.chanpy.KLine.KLine_Unit import CKLine_Unit
from quantlab._vendor.chanpy.Math.TrendLine import CTrendLine
...
class CSeg(Generic[LINE_TYPE]):
    def __init__(self, idx: int, start_bi: LINE_TYPE, end_bi: LINE_TYPE, is_sure=True, seg_dir=None, reason="normal"):
        ...
        from quantlab._vendor.chanpy.ZS.ZS import CZS
        ...
        from quantlab._vendor.chanpy.BuySellPoint.BS_Point import CBS_Point
    ...
    def cal_macd_metric(self, macd_algo, is_reverse):
        if macd_algo == MACD_ALGO.FULL_AREA:
            # Quantlab extension: integrate matching-sign histogram over observed
            # endpoint bars, exactly as CBi.Cal_MACD_area does for a stroke.
            current=self.get_begin_klu();end=self.get_end_klu();area=1e-7
            while current is not None and current.idx<=end.idx:
                histogram=current.macd.macd
                if (self.is_up() and histogram>0) or (self.is_down() and histogram<0):area+=abs(histogram)
                if current.idx==end.idx:return area
                current=current.next
            raise CChanException('Missing observed bars in segment MACD area', ErrCode.PARA_ERROR)
