...
class ClassicWyckoffFactor(ComputedFactor):
    ...
    def matrix(self,bars,parameters):
        ...
        for number,(_,group) in enumerate(bars.group_by('symbol',maintain_order=True),1):
            ...
            rows=group.to_dicts();columns={k:[] for k in COMPONENTS};state=None
            ...
            parts.append(group.select('symbol','datetime','available_at').with_columns(*[pl.Series(k,x,dtype=pl.Float64) for k,x in columns.items()]))
