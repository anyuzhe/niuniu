...
Playbook Lab 只读查询正式宿主记录：先看ExpertSource与PlaybookDefinition，再核对PlaybookCase的完整CandidateSet和SelectionDecision。研究“10选2”必须保留未选候选；FULL/STRICT_PIT/FROZEN/VERIFIED任一缺失都不得称正式样本外验证。PlaybookValidation的alpha_verified固定不是盈利认证，执行收益还要核对T+1、涨跌停、停牌、费用和滑点审计。模型没有写Playbook对象的工具。
