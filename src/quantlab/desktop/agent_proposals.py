...
from PyQt6.QtWidgets import QDialog,QVBoxLayout,QPlainTextEdit,QListWidget,QListWidgetItem,QCheckBox,QPushButton
...
class ProposalDialog(QDialog):
    def __init__(self, window, selected_id=None):
        ...
        box.addWidget(button('使用原业务表单填写提案草稿',self.edit_form))
        ...
        self.status=label('只预检不会读取行情；批准时不冻结行情字节，执行时由原引擎保存快照。','muted',True);box.addWidget(self.status)
