...
### P13 Paper → Real 的渐进式交易层（P13-A 已完成；P13-B+ 需单独评审）
...
- P13-A 明确拒绝密码/Token/API Key/Cookie/Session/真实券商账号等敏感字段；System Health 即使看到 Broker Snapshot 也保持 `real_broker_connected=false`。
- **P13-B+ 开始前仍必须单独冻结**：具体券商/账户范围、认证与密钥存放、允许订单类型、资金/单票/日损/总敞口上限、T+1/停牌/涨跌停约束、人工确认点、kill switch、审计/回执/对账和故障恢复。
- 不得把 Paper fill、Mobile 按钮、AI Decision、Shadow MATCH 或 Agent 共识直接映射为真实订单；任何真实下单必须是后续显式启用的独立能力，默认关闭。
...
P13-A 已完成只读账户证据与 Shadow 对账后，正式产品路线只剩真实连接与订单权限边界：

1. **P13-B+ Real Broker**：具体券商实时只读连接、认证/密钥、真实资金和任何订单能力继续单独评审；默认关闭，不因 Broker Snapshot / Shadow MATCH 自动升级。
