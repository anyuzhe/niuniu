...
3. **Strict PIT Evidence Archive + Coverage v1 已完成；真实官方资料继续补齐（当前内部主线）**：PIT Universe / security_status / industry / daily market cap 均需权威 URL、本地官方原文 SHA256 与确认 publication_at 的 receipt；Coverage 只做 evidence presence 与回顾性 inventory，不生成总完成率。SecurityStatus 第二批后已有14条/7只股票，其它三类仍为0；另有1个 MarketRules v2 verified snapshot/7个明确停牌 session。7个复牌日 exact 算术值已由官方前收+公告比例+规则公式核对，但历史行情响应只在事后取得，故单独留作非资格 reference，未追加 MarketRules。当前优先建设 PIT Universe/连续状态链，并继续寻找开盘前静态参数历史文件。
