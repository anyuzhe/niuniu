...
- 全集只能由exact-session PIT Universe receipt认证；每日证券状态完整性只能由绑定该Universe全部members的SecurityStatus v2认证。稀疏status statement不得跨日传播；进入/持续/撤销只可从显式相邻snapshot链推导，root、断链、同日歧义、分叉或Universe进出不得冒充状态变化。
