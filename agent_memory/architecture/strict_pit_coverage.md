...
- 四类 statement 维度固定为 `universe_eligibility / security_status / industry_membership / daily_market_cap`；完整股票池另列 `pit_universe_archive`，不得用前者的零散计数替代全集 snapshot。
...
- 当前 `/Volumes/Lexar/niuniu-data` 已有 `security_status=14` verified receipts（7只股票），其它三类 statement 与 PIT Universe v1 真实 snapshot 仍为 0；历史 bars 很多不改变覆盖不完整这一事实。
