...
用户于 2026-09-17 明确纠正：研究必须由牛牛助手自行读取数据、形成假设、调用研究工具、解释并保存结果。外部开发者负责观察正式入口的调用与产物，发现功能性数据缺口或流程问题才补代码，再让牛牛验证。

不得将开发者自选因子/控制变量、手工扩样统计或临时脚本代跑，冒充牛牛自主研究。测试可以约束数据副本、权限、预算和停止条件，但不提前提供研究答案；测试独有工具不能代替产品接线。既有研究脚本只作明确的人工验证/复算用途，不作为后续默认主线。验收关注工具、任务、记忆、失败反馈和权限，不以挖出盈利因子作为通过条件。
